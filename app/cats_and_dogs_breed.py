...
# def cat_and_dog_breed_detector_app(key="breed_detector"):
#     #######################################################################################################################

# pets_breed_learner = load_learner(fname="models/pets_breed_learner.pkl")

# with open(file="models/pets_breed_vocab.pkl", mode="rb") as f:
#     pets_breed_vocab = pickle.load(f)
# pets_breed_vocab = sorted(pets_breed_vocab)

# #######################################################################################################################

# st.header("🐶🐱 Cat & Dog Breed Detector 🐱🐶")
...
# upload_photo_tab, capture_photo_tab, model_tab = st.tabs(
#     ["**Upload Photo**", "**Capture Photo**",  "**Model Performance**"])

# #######################################################################################################################

# with upload_photo_tab:
#     options = st.multiselect("**Select multiple names at once, then do Google/Bing Search, Download, Upload & Detect ...**",
#                              pets_breed_vocab,
#                              pets_breed_vocab[10:18],
...
#             output = pets_breed_learner.predict(image)
